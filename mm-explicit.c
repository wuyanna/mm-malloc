...
int mm_init(void)
{
    ...
    PUT_ADDR(heap_listp+DSIZE, NULL);            /* root next free node */
...
static void insert_freenode(void *bp)
{
    ...
    PUT_ADDR(NEXTP(bp), *NEXTP(heap_listp));
    PUT_ADDR(PREVP(bp), heap_listp);
    
    PUT_ADDR(NEXTP(heap_listp), bp);
    if (next_free_block_addr != NULL) {
...
static void place(void *bp, size_t asize)
{
    ...
    if ((csize - asize) >= MIN_BLKSIZE) {
        ...
        dbg_printblock(bp);
        PUT_ADDR(NEXTP(bp), next_free_block_addr);
        PUT_ADDR(PREVP(bp), prev_free_block_addr);
        PUT_ADDR(NEXTP(prev_free_block_addr), bp);
        if (next_free_block_addr != NULL) {
            PUT_ADDR(PREVP(next_free_block_addr), bp);
        }
